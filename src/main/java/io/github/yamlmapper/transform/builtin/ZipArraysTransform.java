...
public class ZipArraysTransform implements Transform {
  ...
  public JsonNode apply(JsonNode input, TransformContext context) {
    ...
    JsonNode rootNode = context.getRootNode();
    if (rootNode == null) {
      return input;
    }
    ...
    for (JsonNode item : input) {
      ...
      for (Map.Entry<String, String> entry : mergeMap.entrySet()) {
        ...
        JsonNode sourceNode = resolveSourcePath(rootNode, sourcePath);
  ...
  /**
   * Resolves a dot-notation path from the root node.
   */
  private JsonNode resolveSourcePath(JsonNode root, String path) {
