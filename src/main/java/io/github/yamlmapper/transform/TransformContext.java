...
public interface TransformContext {
  ...
  /**
   * Resolves a JSON path from the root node.
   *
   * <p>This method uses the cached PathResolver for efficient path resolution.
   * Supports dot-notation and array indexing (e.g., "user.address.city", "items[0].name").
   *
   * <p>Example usage:
   * <pre>{@code
   * // Instead of manually splitting paths:
   * JsonNode prices = context.resolvePath("pricing.discounts");
   * }</pre>
   *
   * @param path the JSON path to resolve (e.g., "user.address", "items[0]")
   * @return the resolved JsonNode, or null if path doesn't exist
   */
  JsonNode resolvePath(String path);
}
