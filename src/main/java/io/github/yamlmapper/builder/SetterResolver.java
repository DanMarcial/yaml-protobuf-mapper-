...
public class SetterResolver {
  ...
  // Sentinel value for "not a oneof field" to distinguish from uncached
  private static final OneofInfo NOT_A_ONEOF = new OneofInfo("", "");

  // Cache for setter MethodHandles - uses record key to avoid String allocation
  private final Cache<CacheKey, SetterInfo> setterCache;

  // Cache for oneof info - avoids repeated descriptor lookups
  private final Cache<CacheKey, OneofInfo> oneofCache;
  ...
  public SetterResolver() {
    ...
    this.oneofCache = CacheFactory.create(CacheConfig.SETTER_CACHE);
  }
  ...
  private SetterInfo getOrCreateSetterInfo(final Message.Builder builder, final String fieldName) {
    CacheKey cacheKey = buildCacheKey(builder.getClass(), fieldName, MethodType.SET);
  ...
  private SetterInfo getOrCreateAdderInfo(final Message.Builder builder, final String fieldName) {
    CacheKey cacheKey = buildCacheKey(builder.getClass(), fieldName, MethodType.ADD);
  ...
  private SetterInfo getOrCreateMapSetterInfo(final Message.Builder builder, final String fieldName) {
    CacheKey cacheKey = buildCacheKey(builder.getClass(), fieldName, MethodType.PUT_ALL);
    return setterCache.get(cacheKey, k -> createMapSetterInfo(builder, fieldName));
  }

  /**
   * Method type for cache key differentiation.
   */
  private enum MethodType { SET, ADD, PUT_ALL }

  /**
   * Cache key record - avoids String concatenation overhead.
   * Records have efficient hashCode/equals implementations.
   */
  private record CacheKey(Class<?> builderClass, String fieldName, MethodType methodType) {}

  private CacheKey buildCacheKey(final Class<?> builderClass, final String fieldName, final MethodType methodType) {
    return new CacheKey(builderClass, fieldName, methodType);
  }
  ...
  public OneofInfo getOneofInfo(final Message.Builder builder, final String fieldName) {
    CacheKey cacheKey = buildCacheKey(builder.getClass(), fieldName, MethodType.SET);
    OneofInfo cached = oneofCache.get(cacheKey, k -> computeOneofInfo(builder, fieldName));
    // Return null for NOT_A_ONEOF sentinel
    return cached == NOT_A_ONEOF ? null : cached;
  }

  private OneofInfo computeOneofInfo(final Message.Builder builder, final String fieldName) {
