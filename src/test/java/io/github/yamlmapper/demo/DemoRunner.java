...
public class DemoRunner {
    ...
    private static final String[][] SCENARIOS = {
        ...
        {"10", "CustomAttributes", "Map fields to CustomAttribute objects"},
    };
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (running) {
            ...
            switch (choice) {
                ...
                case "10" -> runScenario10_CustomAttributes();
    ...
    private static void runAllScenarios() throws Exception {
        ...
        runScenario10_CustomAttributes();
    ...
    private static void runScenario10_CustomAttributes() throws Exception {
        printScenarioHeader(10, "CustomAttributes",
                "Map JSON fields to Protobuf CustomAttribute objects");

        System.out.println(box(
                "CustomAttribute is a Protobuf type used for flexible key-value attributes.\n" +
                "It supports both text[] and numbers[] values.\n\n" +
                "Two transforms handle CustomAttribute creation:\n" +
                "  • fieldsToAttributeMap - converts named fields to attribute map\n" +
                "  • parseKeyValuePairs  - parses delimited strings like \"Key:Value|Key:Value\""));

        // ─────────────────────────────────────────────────────────────────────────
        // PART A: fieldsToAttributeMap Transform
        // ─────────────────────────────────────────────────────────────────────────
        System.out.println("\n" + CYAN + "═══════════════════════════════════════════════════════════════" + RESET);
        System.out.println(CYAN + "▸ PART A: fieldsToAttributeMap Transform" + RESET);
        System.out.println(CYAN + "═══════════════════════════════════════════════════════════════" + RESET);

        String yamlFieldsToAttr = """
            # YAML Configuration: fieldsToAttributeMap
            # Converts specific JSON fields to a map<string, CustomAttribute>
            ...
            fields:
              ...
              attributes:
                type: map
                source: [.]                    # Root object as source
                objectType: CustomAttribute
                transform: fieldsToAttributeMap
                transformParams:
                  fields:
                    - items:group_ids          # Array field → text[]
                    - items:keywords           # Array field → text[]
                    - variations:facet:availability
            """;

        String jsonFieldsToAttr = """
            {
              "items:id": "PROD-12345",
              "items:group_ids": ["GROUP-A", "GROUP-B", "GROUP-C"],
              "items:keywords": ["laptop", "gaming", "portable"],
              "variations:facet:availability": ["in_stock", "online"]
            }
            """;

        printYamlCompact(yamlFieldsToAttr);
        System.out.println("\n" + CYAN + "▸ Input JSON:" + RESET);
        printBoxedContent(jsonFieldsToAttr.trim(), 60);

        // Execute mapping
        MappingEngine engineA = MappingEngine.builder()
                .withProtobufPackage(PROTOBUF_PACKAGE)
                .withConfig("classpath:integration/mapping/product-attributes.yaml")
                .injectEventType(false)
                .build();

        JsonNode jsonNodeA = MAPPER.readTree(jsonFieldsToAttr);
        Product productA = engineA.map(jsonNodeA, "product-attributes", Product.class);

        System.out.println("\n" + CYAN + "▸ Result - Accessing CustomAttributes:" + RESET);
        System.out.println("  product.getId() = " + GREEN + "\"" + productA.getId() + "\"" + RESET);
        System.out.println("  product.getAttributesCount() = " + GREEN + productA.getAttributesCount() + RESET);

        System.out.println("\n" + CYAN + "▸ Individual attributes:" + RESET);
        productA.getAttributesMap().forEach((key, attr) -> {
            if (!attr.getTextList().isEmpty()) {
                System.out.println("  \"" + key + "\" → text: " + GREEN + attr.getTextList() + RESET);
            } else if (!attr.getNumbersList().isEmpty()) {
                System.out.println("  \"" + key + "\" → numbers: " + GREEN + attr.getNumbersList() + RESET);
            }
        });

        // ─────────────────────────────────────────────────────────────────────────
        // PART B: parseKeyValuePairs Transform
        // ─────────────────────────────────────────────────────────────────────────
        System.out.println("\n" + CYAN + "═══════════════════════════════════════════════════════════════" + RESET);
        System.out.println(CYAN + "▸ PART B: parseKeyValuePairs Transform" + RESET);
        System.out.println(CYAN + "═══════════════════════════════════════════════════════════════" + RESET);

        System.out.println("\n" + YELLOW + "▸ Use case:" + RESET + " Legacy systems often encode attributes as delimited strings:");
        System.out.println("  " + DIM + "\"Price:56.99|Brand:Nike|Color:Red|Color:Blue\"" + RESET);

        String yamlParseKV = """
            # YAML Configuration: parseKeyValuePairs
            # Parses delimited strings into CustomAttribute map
            ...
            attributes:
              ...
              transformParams:
                pairDelimiter: "|"        # Separates pairs
                keyValueDelimiter: ":"    # Separates key from value

            # Result: Duplicate keys are grouped into arrays
            # "Color:Red|Color:Blue" → {"Color": {text: ["Red", "Blue"]}}
            # Numeric values auto-detected: "Price:56.99" → {numbers: [56.99]}
            """;

        printYamlCompact(yamlParseKV);

        // Demonstrate the transform directly
        TransformRegistry registry = BuiltinTransforms.createRegistry();
        Transform parseKVTransform = registry.get("parseKeyValuePairs");

        String facetsInput = "Price:56.99|Brand:Nike|Health Feature:Scientific Formula|Health Feature:Joint Support|Weight:29.1";

        System.out.println("\n" + CYAN + "▸ Input string:" + RESET);
        System.out.println("  " + YELLOW + "\"" + facetsInput + "\"" + RESET);

        FieldConfig parseKVConfig = FieldConfig.builder("attributes")
                .transformParams(Map.of(
                        "pairDelimiter", "|",
                        "keyValueDelimiter", ":"
                ))
                .build();
        TransformContextImpl parseKVCtx = TransformContextImpl.builder()
                .params(parseKVConfig.transformParams())
                .objectMapper(MAPPER)
                .build();

        JsonNode inputNode = MAPPER.readTree("\"" + facetsInput + "\"");
        JsonNode resultKV = parseKVTransform.apply(inputNode, parseKVCtx);

        System.out.println("\n" + CYAN + "▸ Output (CustomAttribute structure):" + RESET);
        String prettyResult = MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(resultKV);
        printBoxedContent(prettyResult, 65);

        System.out.println("\n" + YELLOW + "▸ Note:" + RESET + " Numbers are auto-detected:");
        System.out.println("  • \"Price:56.99\" → " + GREEN + "{\"numbers\": [56.99]}" + RESET);
        System.out.println("  • \"Brand:Nike\"  → " + GREEN + "{\"text\": [\"Nike\"]}" + RESET);
        System.out.println("  • Duplicate keys grouped: \"Health Feature\" has 2 values");

        // ─────────────────────────────────────────────────────────────────────────
        // PART C: Merge Definitions (Multiple Sources)
        // ─────────────────────────────────────────────────────────────────────────
        System.out.println("\n" + CYAN + "═══════════════════════════════════════════════════════════════" + RESET);
        System.out.println(CYAN + "▸ PART C: Merge Definitions (Combining Multiple Sources)" + RESET);
        System.out.println(CYAN + "═══════════════════════════════════════════════════════════════" + RESET);

        System.out.println("\n" + YELLOW + "▸ Use case:" + RESET + " Attributes come from multiple places in the JSON:");
        System.out.println("  1. A delimited string field (facets)");
        System.out.println("  2. Specific named fields (metadata:*)");
        System.out.println("  3. Other named fields (facet:*)");

        String yamlMerge = """
            # YAML Configuration: Merge Definitions
            # Use array syntax to combine multiple sources into one field

            attributes:
              # First source: Parse facets string
              - type: map
                source: [facets]
                objectType: CustomAttribute
                transform: parseKeyValuePairs
        ...
        printYamlCompact(yamlMerge);

        String jsonMerge = """
            {
              "facets": "Price:99.99|Material:Cotton",
              "metadata:brand": ["Nike", "Jordan"],
              "metadata:category": ["Shoes", "Running"],
              "facet:color": ["Red", "Blue", "Black"],
              "facet:size": ["S", "M", "L", "XL"]
            }
            """;

        System.out.println("\n" + CYAN + "▸ Input JSON (multiple attribute sources):" + RESET);
        printBoxedContent(jsonMerge.trim(), 55);

        System.out.println("\n" + YELLOW + "▸ Result:" + RESET + " All sources are merged into a single attributes map:");
        System.out.println("  • From parseKeyValuePairs: Price, Material");
        System.out.println("  • From fieldsToAttributeMap #1: metadata:brand, metadata:category");
        System.out.println("  • From fieldsToAttributeMap #2: facet:color, facet:size");

        // Show using complex product
        System.out.println("\n" + CYAN + "▸ Live example with product-complex.yaml:" + RESET);

        MappingEngine engineC = MappingEngine.builder()
                .withProtobufPackage(PROTOBUF_PACKAGE)
                .withConfig("classpath:integration/mapping/product-complex.yaml")
                .injectEventType(false)
                .build();

        String chaoticJson = loadResource("integration/json/product-chaotic.json");
        Product productC = engineC.map(chaoticJson, "product-complex", Product.class);

        System.out.println("  product.getAttributesCount() = " + GREEN + productC.getAttributesCount() + RESET);
        if (productC.getAttributesCount() > 0) {
            System.out.println("  Merged attributes:");
            productC.getAttributesMap().forEach((key, attr) -> {
                String value = !attr.getTextList().isEmpty()
                    ? "text: " + attr.getTextList()
                    : "numbers: " + attr.getNumbersList();
                System.out.println("    \"" + key + "\" → " + GREEN + value + RESET);
            });
        }
